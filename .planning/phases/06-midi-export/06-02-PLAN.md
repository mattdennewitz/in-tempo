...
<task type="auto">
  ...
  <action>
    ...
    2. Update `src/audio/engine.ts`:
       - In `Scheduler.getState()` (in scheduler.ts): add `hasRecording: this.midiRecorder?.eventCount ?? 0 > 0` -- actually this belongs in the Scheduler. Wait, getState() is on Scheduler. So:
       - In `src/audio/scheduler.ts`, in `getState()`: add `hasRecording: (this.midiRecorder?.eventCount ?? 0) > 0`
       - In `src/audio/engine.ts`, in the fallback state returned by `getState()`: add `hasRecording: false`

    3. Create `src/components/ExportButton.tsx`:
    ...
    4. Update `src/App.tsx`:
       ...
       - Add `INITIAL_STATE.hasRecording = false`
